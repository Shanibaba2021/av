...
class Inventory extends CI_Controller
{
	...
	public function assignstock($id = Null)
	{
		...
		$this->form_validation->set_rules('assign_quantity', 'Assign Quantity', 'trim|required|max_length[128]|less_than_equal_to[' . $this->input->post('current_quantity') . ']|greater_than[0]');
		if ($this->form_validation->run() == FALSE) {
	...
	public function branchstock()
	{
		...
		$data['branches']  = $this->Inventory_model->getBranchList('tbl_branches');
		$data['product']  = $this->Inventory_model->getListTable('tbl_product');
		$this->load->view('admin/inventory/branchstock', $data);
	}
	...
	public function checkstock($branchId)
	{
		...
		if (isset($branchId) and !empty($branchId)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data['beneficiary']  = $this->Inventory_model->getListbranchstock('tbl_branches', $branchId);


				$kit_assign = $data['beneficiary'];

				$i = 0;
				foreach ($kit_assign as  $row) {

					$kit_assign[$i]->kit_assign = $this->Inventory_model->getListTableByCol("tbl_assign_item_branch", "bname", $row->branchId);

					$i++;
				}

				$data["results"] = array("kit_assign" => $kit_assign,);


	

				$this->load->view('admin/inventory/checkstockforbranch', $data);
			} else {

				redirect('admin/inventory/checkstock');
			}
		} else {

			$this->session->set_flashdata('message', ' Invalid Id !');

			redirect('admin/inventory/checkstockforbranch');
		}
	}
}
