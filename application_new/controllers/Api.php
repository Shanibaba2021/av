...
class Api extends CI_Controller
{
    ...
    public function getbranches()
    {
        ...
        $branches = $this->api_model->getListTable("tbl_branches");
    ...
    public function get_district()
    {
        ...
        $data["state"] = $this->api_model->get_state("state");

        foreach ($data["state"] as $value) {
            $data["district"] = $this->api_model->get_district("district", $value['StCode']);

            echo json_encode(array("status" => "1", "msg" => "District getting successfully", "data" => $data['district']));
        }

        if ($data > 0) {
            echo json_encode(array("status" => "1", "msg" => "State getting successfully", "data" => $data['state']));
        } else {
            echo json_encode(array("status" => "0", "msg" => "Gallery getting Failed", "data" => []));
        }
    }
    ...
    public function getInsuranceLIst()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->model('Api_model');
            $getInsurance = $this->Api_model->getInsurance("tbl_beneficiary", $fid);
            $data["total_Insurance"] = $getInsurance[0]["total_insurance"];

            $data["insurance_list"] = $this->Api_model->getInsuranceLIst("tbl_beneficiary", $fid);
    ...
    public function updatetrainingsession()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($data > 0) {
                echo json_encode(array("status" => "1", "msg" => "New Beneficiary Successfully Store ", "data" => $data));
            } else {
                echo json_encode(array("status" => "0", "msg" => "New Beneficiary created Failed", "data" => []));
            }
        }
