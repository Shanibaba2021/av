...
class User_model extends CI_Model
{
    ...
    public function get_user_chech_out($user_id = 0)
    {
        if($this->session->userdata('user_id') == 1)
        {
            if ($user_id > 0) {
    ...
    public function add_user($is_instructor = false, $is_admin = false)
    {
        ...
        if ($validity == false) {
            ...
            if($this->session->userdata('user_id') == 1)
            {
                ...
                $data['type_organization'] = html_escape($this->input->post('organization'));
    ...
    public function approve_user($user_id = "")
    {
        ...
        $this->db->where('id', $user_id);
        $this->db->update('users', $application_data);
        $this->session->set_flashdata('flash_message', get_phrase('user_approve_successfully'));
    }
    ...
    public function block_user($user_id = "")
    {
        ...
        $this->db->where('id', $user_id);
        $this->db->update('users', $application_data);
        $this->session->set_flashdata('flash_message', get_phrase('user_block_successfully'));
    }
    ...
    public function get_instructor($id = 0)
    {
        ...
        if($this->session->userdata('user_id') == 1)
        {
            if ($id > 0) {
                ...
                return $this->db->get_where('users', array('is_instructor' => 1,'id !=' => 1,'role_id' => 2,));
            }
        ...
        else
        {
            ...
            if ($id > 0) {
                return $this->db->get_where('users', array('uid' => $id,'is_instructor' => 1));
            } else {
    ...
    {
        if ($id > 0) {
            return $this->db->get_where('users', array('id' => $id, 'role_id' => 1,'user_id' => 1));
        } else {
            return $this->db->get_where('users', array('role_id' => 1,'id !=' => 1));
        }
    }
    ...
    public function get_instructor_data($id = 0)
    {
