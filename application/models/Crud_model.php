...
class Crud_model extends CI_Model
{
    ...
    {
        $query =  $this->db->query("SELECT *  FROM `users` WHERE NOT JSON_SEARCH(uid, 'all', '$where') IS NULL");
    ...
    function getListTable($table)
    {
        ...
        $this->db->where('type_organization', 1);
    ...
    function getUserListTable($table)
    {
        ...
        $this->db->where('type_organization', 2);
        $this->db->where('id !=', 1);
    ...
    function getdatajobTable($table)
    {
    ...
    public function get_jobs($param1 = "")
    {
        ...
        if ($id == 1) {
            $this->db->select('*');
            $this->db->from('jobs');
            $this->db->join('job_category', 'job_category.id = jobs.job_category', 'left');
            $this->db->join('salary_range', 'salary_range.id = jobs.salary_range', 'left');
            $this->db->join('job_experience', 'job_experience.id = jobs.experience', 'left');
            $this->db->join('job_type', 'job_type.id = jobs.job_type', 'left');
            return $this->db->get();
        } else {
            $this->db->select('*');
            $this->db->from('jobs');
    ...
    public function enrol_history($course_id = "")
    {
        if ($course_id > 0) {
            ...
            if ($user_id != 1) {
                ...
                $this->db->where('find_in_set("' . $user_id . '",course.user_id) <> 0');
                return $this->db->get();
            } else {
    ...
    public function get_revenue_by_user_type($timestamp_start = "", $timestamp_end = "", $revenue_type = "")
    {
        ...
        if ($revenue_type == 'admin_revenue') {
            $user_id = $this->session->userdata('user_id');
            if ($user_id == 1) {
                $this->db->where('payment.date_added >=', $timestamp_start);
                $this->db->where('payment.date_added <=', $timestamp_end);
            } else {
                $this->db->join('course', 'course.id = payment.course_id', 'left');
                $this->db->where('payment.date_added >=', $timestamp_start);
                $this->db->where('payment.date_added <=', $timestamp_end);
                $this->db->where('find_in_set("' . $user_id . '",course.user_id) <> 0');
            }
    ...
    public function add_course($param1 = "")
    {
        ...
        $data['instructor_id'] = html_escape($this->input->post('select_instructor'));
        $data['instructor'] = html_escape($this->input->post('instructor'));
        ...
        $instructor_id = $this->input->post('select_instructor');
        $instructor = $this->input->post('instructor');
        $admin_id = $this->session->userdata('user_id');
        $data['user_id'] = "$admin_id,$instructor_id,$instructor";
    ...
    public function sign_up_instructors()
    {
        ...
        if ($validity == false) {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('Sign Up Successfully'));
        }
    }
    ...
    public function sign_up_company()
    {
        ...
        if ($validity == false) {
            ...
            $data['university_name'] = html_escape($this->input->post('university_name'));
            $data['type_organization'] = 1;
            $data['uid'] = 1;
            $data['address'] = html_escape($this->input->post('organization_address'));
            $data['first_name'] = html_escape($this->input->post('first_name'));
            $data['last_name'] = html_escape($this->input->post('last_name'));
            $data['email'] = html_escape($this->input->post('email'));
            $data['password'] = sha1(html_escape($this->input->post('password')));
            ...
            $permission_data['permissions'] = json_encode(["category", "course", "user", "instructor", "student", "enrolment", "revenue", "messaging", "addon", "theme", "settings", "coupon"]);
            $this->db->insert('permissions', $permission_data);
            $this->session->set_flashdata('flash_message', get_phrase('Sign Up Successfully'));
        }
    }

    public function sign_up_university()
    {
        $validity = $this->check_duplication('on_create', $this->input->post('email'));
        if ($validity == false) {
            $this->session->set_flashdata('error_message', get_phrase('email_duplication'));
    ...
    public function add_jobs($param1 = "")
    {

        $data['mobile'] = html_escape($this->input->post('mobile'));
        $data['short_description'] = html_escape($this->input->post('job_company'));
        ...
        $data['salary_range'] = html_escape($this->input->post('salary_range'));
        $data['no_of_vacancy'] = html_escape($this->input->post('no_vacancy'));
        $data['experience'] = $this->input->post('exp');
    ...
    public function update_jobs($job_id)
    {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('job_update_successfully'));
    }
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        // $data['instructor_id'] = $this->input->post('select_instructor');
        $data['short_description'] = html_escape($this->input->post('short_description'));
        $data['description'] = $this->input->post('description');
        $data['outcomes'] = $outcomes;
        $data['language'] = $this->input->post('language_made_in');
    ...
    public function get_jobs_for_frontend()
    {
        $this->db->select('*,users.university_name as company_name');
        $this->db->from('jobs');
        $this->db->join("users", "jobs.company_id = users.id", "left");
        $this->db->order_by("job_id", "desc");
        $query = $this->db->get();
        return $query->result();
    }
    public function get_companies_for_frontend()
    {
        $this->db->select('*');
        $this->db->from('users');
    ...
    public function get_universities_for_frontend()
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->order_by("id", "desc");
    ...
    public function get_company_details_for_fronted_by_id($id)
    {
        $this->db->select('*');
        $this->db->from('users');
        $this->db->where('id', $id);
    ...
    public function get_jobs_details_for_fronted($job_id)
    {
        $this->db->select('*,users.university_name as company_name');
        $this->db->from('jobs');
        $this->db->join("users", "jobs.company_id = users.id", "left");
    ...
    function get_single_jobs($job_id)
    {
        $this->db->select('*,jobs.skills as skills,users.university_name as company_name');
        $this->db->from('jobs');
        $this->db->join("users", "jobs.company_id = users.id", "left");
        $this->db->where('job_id', $job_id);
        $query = $this->db->get();
    ...
    public function get_jobs_for_company($company_id)
    {
        $this->db->select('*,users.university_name as company_name');
        $this->db->from('jobs');
        $this->db->join("users", "jobs.company_id = users.id", "left");
    ...
    public function delete_job($job_id = "")
    {
        $this->db->where('job_id', $job_id);
        $this->db->delete('jobs');
        $this->session->set_flashdata('flash_message', get_phrase('job_delete_successfully'));
    }
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "")
    {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if (check_user_level($course_details['creator']) == "admin") {
    ...
    public function multi_instructor_course_ids_for_an_instructor($instructor_id)
    {
        ...
        if ($this->session->userdata('user_id') == 1) {
            ...
            $multi_instructor_courses = $this->db->get('course')->result_array();
        } else {
