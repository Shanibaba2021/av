...
class Admin extends CI_Controller
{
    ...
    {
        $user_id = $this->input->post('select_instructor');
        $result =  $this->crud_model->getusers($user_id);
        if ($result) {
    ...
    public function instructor_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_instructor_form') {
            ...
            $page_data['university'] = $this->crud_model->getUserListTable("users");
            $this->load->view('backend/index', $page_data);
    ...
    public function jobs_action($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            $job_id = $this->crud_model->add_jobs();
            redirect(site_url('admin/dashboard/' . $job_id), 'refresh');
        } elseif ($param1 == 'add_shortcut') {
            echo $this->crud_model->add_shortcut_course();
        } elseif ($param1 == "edit") {
            $this->crud_model->update_jobs($param2);
            redirect(site_url('admin/jobs/jobs_edit/' . $param2));
        } elseif ($param1 == 'delete') {
            $job_id = $this->crud_model->delete_job($param2);
            redirect(site_url('admin/get_jobs/' . $job_id), 'refresh');
        }
    }
    ...
    public function jobs($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_jobs') {
            $page_data['page_name'] = 'add_jobs';
            $page_data['page_title'] = get_phrase('add_jobs');
            $page_data['job_category'] = $this->crud_model->getdatajobTable("job_category");
            $page_data['salary_range'] = $this->crud_model->getdatajobTable("salary_range");
            $page_data['job_experience'] = $this->crud_model->getdatajobTable("job_experience");
            $page_data['job_type'] = $this->crud_model->getdatajobTable("job_type");
            $page_data['company'] = $this->crud_model->getListTable("users");
            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == 'add_course_shortcut') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $this->load->view('backend/admin/course_add_shortcut', $page_data);
        } elseif ($param1 == 'jobs_edit') {

            $page_data['page_name'] = 'job_edit';
            $page_data['job_id'] =  $param2;
            $page_data['page_title'] = get_phrase('job_edit');
            $page_data['job_category'] = $this->crud_model->getdatajobTable("job_category");
            $page_data['salary_range'] = $this->crud_model->getdatajobTable("salary_range");
            $page_data['job_experience'] = $this->crud_model->getdatajobTable("job_experience");
            $page_data['job_type'] = $this->crud_model->getdatajobTable("job_type");
            $page_data['get_single_jobs'] = $this->crud_model->get_single_jobs($param2);
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function get_jobs()
    {
        ...
        $page_data['page_name'] = 'get_jobs';
        $page_data['page_title'] = get_phrase('jobs');
        $page_data['jobs'] = $this->crud_model->get_jobs();
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function course_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_course') {
            $page_data['university'] = $this->user_model->get_university()->result_array();
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
    ...
    public function admins($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            ...
            $this->user_model->approve_user($param2);
            redirect(site_url('admin/admins'), 'refresh');
        } elseif ($param1 == "block") {
            // CHECK ACCESS PERMISSION
            check_permission('admin');

            $this->user_model->block_user($param2);
            redirect(site_url('admin/admins'), 'refresh');
        }
    ...
    public function remove_an_instructor($course_id, $university_id)
    {
        // CHECK ACCESS PERMISSION
        check_permission('course');
    ...
    public function remove_an_university($user_id, $university_id)
    {

        $course_details = $this->crud_model->get_user_by_id($user_id)->row_array();

        if ($course_details['id']) {
            $instructor_ids = json_decode($course_details['uid']);
            if (in_array($university_id, $instructor_ids)) {
                if (count($instructor_ids) > 1) {
                    if (($key = array_search($university_id, $instructor_ids)) !== false) {
                        unset($instructor_ids[$key]);

                        $data['uid'] = json_encode($instructor_ids);

                        $this->db->where('id', $user_id);
                        $this->db->update('users', $data);

                        $this->session->set_flashdata('flash_message', get_phrase('university_has_been_removed'));
                        redirect('admin/instructor_form/edit_instructor_form/' . $user_id);
                    }
                } else {
                    $this->session->set_flashdata('error_message', get_phrase('a_instructor_should_have_at_least_one_university'));
                    redirect('admin/instructor_form/edit_instructor_form/' . $user_id);
                }
            } else {
                $this->session->set_flashdata('error_message', get_phrase('invalid_university_id'));
                redirect('admin/instructor_form/edit_instructor_form/' . $user_id);
            }
        } else {
            $this->session->set_flashdata('error_message', get_phrase('a_instructor_should_have_at_least_one_university'));
