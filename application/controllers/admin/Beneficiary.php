...
class beneficiary extends CI_Controller
{
	...
	public function edit($bId='',$insurance_id='')
	{
		...
		if (isset($bId) and !empty($bId)) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data["session"] = $this->Beneficiary_model->getListTable("tbl_session");
				...
				if($insurance == 1 || $insurance == 2 )
				{
					...
					$saveData['father_husband_name'] = '';
				}
				$relation_with_insured = $this->input->post('relation_with_insured');
				if ($relation_with_insured == 6) {
					$saveData['relation_with_insured_name'] = set_value('relation_with_insured_name');
				} else {
					$saveData['relation_with_insured_name'] = '';
				}
				...
				if($tsession != 0 )
				{
					...
					$saveData['tsession'] = 0;
					$saveData['training_id'] = '';
				}

				$other_insurance = $this->input->post('other_insurance');
				if($other_insurance == 1 )
				{
					$saveData['other_insurance'] = set_value('insurance_type');
					$saveData['insurance_company'] = set_value('insurance_company');
					$saveData['insurance_amount'] = set_value('insurance_amount');
				}
				else{
					$saveData['other_insurance'] = 0;
					$saveData['insurance_company'] = '';
					$saveData['insurance_amount'] = '';
				}
	...
	function approvebeneficiary()
	{
		...
		$beneficiaryInfo = array('status' => 2,'note'=>$saveData);

		$this->load->model('Beneficiary_model');
		$result = $this->Beneficiary_model->approvebeneficiary($bId, $beneficiaryInfo);
	...
	function delete($bId ='')
	{
		...
		if (isset($bId) and !empty($bId)) {
			...
			if (isset($count) and !empty($count)) {
				...
				$this->session->set_flashdata('success', 'Beneficiary Deleted Successfully !');
