...
class Home extends CI_Controller
{
    ...
    {
        $this->crud_model->sign_up_instructors();
    ...
    {
        //dasdsdasd
        $page_data['page_name'] = "become_company";
        $page_data['page_title'] = site_phrase('become_company');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function become_university()
    {
        //dasdsdasd
        $page_data['page_name'] = "become_university";
        $page_data['page_title'] = site_phrase('become_university');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function sign_up_company()
    {
        $company_id = $this->crud_model->sign_up_company();
        redirect(site_url('/' . $company_id), 'refresh');
    }
    public function sign_up_university()
    {
        $company_id = $this->crud_model->sign_up_university();
        redirect(site_url('/' . $company_id), 'refresh');
    }
    ...
    public function companies()
    {
        ...
        $page_data['page_title'] = site_phrase('companies');
        $page_data['data'] = $this->crud_model->get_companies_for_frontend();
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function universities()
    {
        $page_data['page_name'] = "universities";
        $page_data['page_title'] = site_phrase('universities');
        $page_data['data'] = $this->crud_model->get_universities_for_frontend();
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function jobs()
    {
        ...
        $page_data['data'] = $this->crud_model->get_jobs_for_frontend();
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function comapny_details($id = '')
    {
        $page_data['page_name'] = "company_details";
        $page_data['page_title'] = site_phrase('company_details');
        $page_data['company_details'] = $this->crud_model->get_company_details_for_fronted_by_id($id);
        $company_id = $page_data['company_details'][0]->id;
        $page_data['get_jobs_for_company'] = $this->crud_model->get_jobs_for_company($company_id);
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function jobs_details($job_id = '')
    {
        $page_data['page_name'] = "jobs_details";
        $page_data['page_title'] = site_phrase('jobs_details');
        $page_data['jobs_details'] = $this->crud_model->get_jobs_details_for_fronted($job_id);
        $company_id = $page_data['jobs_details'][0]->company_id;
        $page_data['get_jobs_for_company'] = $this->crud_model->get_jobs_for_company($company_id);
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function profile($param1 = "")
    {
        ...
        $page_data['edit_data'] = $this->user_model->get_user_profile($id);
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function razorpay_checkout($payment_request = "only_for_mobile")
    {
        ...
        $total_price_of_checking_out = $this->session->userdata('total_price_of_checking_out');
        $page_data['payment_request'] = $payment_request;
        $page_data['user_details']    = $this->user_model->get_user_chech_out($this->session->userdata('user_id'))->row_array();
        $page_data['amount_to_pay']   = $total_price_of_checking_out;
