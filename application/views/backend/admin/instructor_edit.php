...
// $get_user_by_id = $this->crud_model->get_user_by_id($course_id)->row_array();
// pr($user_data);
?>
<div class="row ">
    <div class="col-xl-12">
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <?php if ($this->session->userdata('user_id') == 1) : ?>
                                            <div class="form-group row mb-3">
                                                <label class="col-md-3 col-form-label" for="existing_instructors"><?php echo get_phrase('course_for_the_university'); ?></label>
                                                <div class="col-md-9">
                                                    <?php if ($user_data['id']) :
                                                        $instructor_ids = json_decode($user_data['uid']);
                                                    ?>
                                                        <?php foreach ($instructor_ids as $instructor_id) :
                                                        ?>
                                                            <?php $instructor_details = $this->user_model->get_university_data($instructor_id)->row_array();
                                                            ?>
                                                            <div class="m-2">
                                                                ...
                                                                <span style="font-weight: 700; font-size: 15px; vertical-align: sub; margin-left: 6px;">
                                                                    <?php echo html_escape($instructor_details['university_name']); ?>
                                                                </span>
                                                                <?php if (count($instructor_ids) > 1 && $user_data['id'] != $instructor_id) : ?>
                                                                    <a href="javascript:void(0)" onclick="confirm_modal('<?php echo site_url('admin/remove_an_university/' . $user_data['id'] . '/' . $instructor_details['id']); ?>');" style="display: inline-block; height: 20px; width: 20px; border-radius: 50%; background-color: rgb(239,83,80); color: white; margin-left: 6px; vertical-align: sub;"> <i class="mdi mdi-window-close" style="margin-left: 3px; font-weight: bold; vertical-align: middle;"></i> </a>
                                                                <?php endif; ?>
                                                            </div>
                                                        <?php endforeach; ?>
                                                    <?php else : ?>
                                                        <?php $instructor_details = $this->user_model->get_instructor($user_data['id'])->row_array();?>
                                                        <div>
                                                            <img class="rounded-circle" src="<?php echo $this->user_model->get_user_image_url($instructor_details['id']);; ?>" height="30px" alt="">
                                                            <span style="font-weight: 700; font-size: 15px; vertical-align: sub; margin-left: 6px;">
                                                                <?php echo html_escape($instructor_details['university_name']); ?>
                                            ...
                                            <div class="form-group row mb-3">
                                                <label class="col-md-3 col-form-label" for="last_name"><?php echo get_phrase('university'); ?><span class="required">*</span></label>
                                                <div class="col-md-9">
                                                    <select class="select2 form-control select2-multiple" data-toggle="select2" multiple="multiple" name="uid[]" id="uid">
